package com.example.admin.peacezone.view;
...
import com.example.admin.peacezone.fragment.AddressBookFragment;

/**
 * Created by tangyangkai on 16/7/26.
 */
public class Address_SlideView extends View {
    ...
    public Address_SlideView(Context context) {
        this(context, null);
    }

    public Address_SlideView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Address_SlideView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < AddressBookFragment.pinyinList.size(); i++) {
            String textView = AddressBookFragment.pinyinList.get(i);
            if (i == position - 1) {
                paint.setColor(getResources().getColor(R.color.error_color));
                selectTxt = AddressBookFragment.pinyinList.get(i);
            ...
            mTextHeight += mHeight / AddressBookFragment.pinyinList.size();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                mTextHeight = mHeight / AddressBookFragment.pinyinList.size();
                position = y / (mHeight / (AddressBookFragment.pinyinList.size() + 1));
                invalidate();
                break;
            case MotionEvent.ACTION_MOVE:
                if (isSlide) {
                    backgroundColor = getResources().getColor(R.color.font_text);
                    mTextHeight = mHeight / AddressBookFragment.pinyinList.size();
                    position = y / (mHeight / AddressBookFragment.pinyinList.size() + 1) + 1;
                    invalidate();
                }
                break;
            case MotionEvent.ACTION_UP:
